...
//        if ~ else 문 : 조건식의 결과가 참일 경우 바로 아래의 코드블럭을 실행하고, 결과가 거짓일 경우 else 아래의 코드 블럭을 실행하는 형태
//        단순 if문과는 다르게 반드시 둘 중 하나를 선택하는 형태의 조건문
        ...
        // equals(문자열) : 지정한 문자열과 동일한 문자열인 확인, true/false로 출력
        //if (input.equals("부먹"))
...
//        조건식을 여러개 사용하기 때문에 단계별로 조건식을 설정해야 함(문법적으로 문제가 없으나, 논리적으로 문제가 발생)

//    * 마지막에 존재하는 else 문은 생략이 가능하다
//      1. else 구문으로 갈 수 없도록 모든 조건을 다 입력했을 경우
//      2. else 문으로 빠지는 경우에 아무런 처리를 하지 않을 경우
...
//        중첩 if문 : if, if ~ else, else if 문의 코드블럭에 if문을 사용하는 방식
        ...
        if (grade2 >= 90) {
            System.out.println("당신의 점수는 " + grade2 + "점이고, 등급은 A 입니다.");
        }
        else {
            if (grade2 >= 80) {
                System.out.println("당신의 점수는 " + grade2 + "점이고, 등급은 B 입니다.");
            }
            else {
                if (grade2 >= 70) {
                    System.out.println("당신의 점수는 " + grade2 + "점이고, 등급은 C 입니다.");
                }
                else {
                    if (grade2 >= 60) {
                        System.out.println("당신의 점수는 " + grade2 + "점이고, 등급은 D 입니다.");
                    }
                    else {
                            System.out.println("당신의 점수는 " + grade2 + "점이고, 등급은 F 입니다.");
                        }
                    }
                }
            }
